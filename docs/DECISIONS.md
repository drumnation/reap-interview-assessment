...
## D007: Test Strategy — Source Analysis Over DOM/HTTP Testing

**Decision**: Transaction tests read source files and verify code patterns (presence of `updateMany`, absence of individual fetch loops) rather than spinning up a Next.js server or rendering React components.

**Context**: Setting up MSW + JSDOM + React Testing Library for Next.js App Router is substantial setup work for an assessment. The bugs are structural — wrong endpoint, wrong Prisma method, wrong data flow — and can be verified by reading the source.

**Alternatives**:
- Full integration tests with HTTP: More realistic but requires test DB, server startup, seed data
- React component tests with RTL: Requires JSDOM, mocking fetch, rendering the full page
- E2E with Playwright: Gold standard but significant setup

**Trade-off**: Source-reading tests are brittle to refactoring (pattern-matching on code text). Acceptable for an assessment where the goal is to demonstrate the bug existed and was fixed.

**Files changed**: `src/__tests__/transactions.test.ts`
